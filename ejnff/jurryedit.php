...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['file']['name'])) {
        ...
        $insert = mysqli_query($conn, "UPDATE `jurry` SET `title`= '" . $_POST['title'] . "',`description`='" . $_POST['description'] . "',`image`='" . $file . "' WHERE id='" . $_GET['id'] . "'");
        //var_dump($insert);


    } else {
        $insert = mysqli_query($conn, "UPDATE `jurry` SET `title`= '" . $_POST['title'] . "',`description`='" . $_POST['description'] . "'WHERE id='" . $_GET['id'] . "'");
    }
    if ($insert) {
        header('location:jurry.php');
    }
}



$sel_news = mysqli_query($conn, "SELECT * from jurry where id='" . $_GET['id'] . "' ");
