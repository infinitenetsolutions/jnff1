...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['file']['name'])) {
        $targetfolder = "well_wishing/";
        $file = $_FILES['file']['name'];
        move_uploaded_file($_FILES['file']['tmp_name'], $targetfolder . $file);



        $insert = mysqli_query($conn, "UPDATE `well_wishing` SET `title`= '" . $_POST['title'] . "',`description`='" . $_POST['description'] . "',`image`='" . $file . "' WHERE id='" . $_GET['id'] . "'");
        //var_dump($insert);


    } else {
        $insert = mysqli_query($conn, "UPDATE `well_wishing` SET `title`= '" . $_POST['title'] . "',`description`='" . $_POST['description'] . "'WHERE id='" . $_GET['id'] . "'");
    }
    if ($insert) {
...
$sel_news = mysqli_query($conn, "SELECT * from well_wishing where id='" . $_GET['id'] . "' ");
    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="row pad-botm">
                <div class="col-md-12">
                    <h4 class="header-line">Well Wishing</h4>
